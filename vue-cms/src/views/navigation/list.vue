    ...
    <el-col :span="16" :offset="4">
      <el-card class="box-card" shadow="hover" style="margin-top:30px;">
        <div slot="header" class="clearfix">
          ...
          <el-button style="float: right; padding: 3px 10px 3px 0" type="text" @click="getNavTree">刷新</el-button>
        </div>
    ...
    handleDrop(draggingNode, dropNode, dropType, ev) {
      ...
      switch (dropType) {
        case 'before':
          ...
          position = dropNode.data.order - 1
          break
        case 'after':
          parentId = dropNode.data.parentId
          position = dropNode.data.order + 1
      ...
      this.loading = true
      moveNav(draggingNode.data.id, parentId, position).then(response => {
        this.getNavTree()
      }).catch((error) => {
        this.loading = false
