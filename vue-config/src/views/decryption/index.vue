      ...
      <el-col :span="12" :offset="5">
        <el-card :body-style="{ padding: '30px' }">
          <el-form ref="formModel" :model="temp" :rules="rules" label-position="right" label-width="120px" style="margin-left:10px;margin-right:10px;">
            <el-form-item label="文件名称:" prop="oldName"><el-input v-model="temp.oldName" :disabled="!canUpload" name="oldName" /></el-form-item>
          ...
          <div style="padding: 14px;">
            <div class="bottom clearfix">
              <el-button v-if="canUpload" type="primary" class="button" @click="submitUpload">上传解密</el-button>
              <template v-else>
                <el-link type="success" icon="el-icon-download" :href="download + '?name=' + temp.oldName + '&key='+ key">下载解密文件</el-link>
...
export default {
  data() {
    return {
      url: process.env.VUE_APP_BASE_API + '/Encryption/UploadOldConfig',
      download: process.env.VUE_APP_BASE_API + '/Encryption/GetFile',
      temp: { oldName: 'appsettings.json', fileName: 'app.ept', fileList: [] },
      rules: {
  ...
  methods: {
    onSuccess(data) {
      ...
      this.$notify({ title: '成功', message: '已解密，请点击下载', type: 'success', duration: 2000 })
    },
    submitUpload() {
