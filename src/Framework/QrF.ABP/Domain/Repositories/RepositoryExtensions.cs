...
namespace QrF.ABP.Domain.Repositories
{
    public static class RepositoryExtensions
    {
        ...
        public static IIocResolver GetIocResolver<TEntity, TPrimaryKey>(this IRepository<TEntity, TPrimaryKey> repository)
            where TEntity : class, IEntity<TPrimaryKey>
        {
            var repo = ProxyHelper.UnProxy(repository) as RepositoryBase<TEntity, TPrimaryKey>;
            if (repo != null)
            {
                return repo.IocResolver;
            }

            throw new ArgumentException($"Given {nameof(repository)} is not inherited from {typeof(RepositoryBase<TEntity, TPrimaryKey>).AssemblyQualifiedName}");
        }
    }
}
